...
`tags` 可填写一个或多个标签；文章页和首页会显示标签。点击 `JVM` 会进入 `/tags/JVM/`，只查看带有该标签的文章。

新增标签时，还需在 `tags/` 下创建对应的 Markdown 页面，设置 `layout: tag`、`tag: 标签名` 和 `permalink: /tags/标签名/`。已有的 `JVM.md` 和 `JNI.md` 可作为示例。
